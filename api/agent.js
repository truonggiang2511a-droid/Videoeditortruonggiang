...
const fallback = ({ prompt = '', videoMeta = {}, assets = [], businessRules = {} } = {}) => {
  ...
  const ordered = assets.slice(0, 30).map((a, i) => ({ assetId: a.id || a.name || String(i), assetName: a.name, storage_path: a.storage_path, start: 0, end: Math.min(4, Number(videoMeta.duration || 4)), role: i === 0 ? 'hook' : 'broll', score: 100 - i * 2, reason: 'Fallback asset ranking' }));
  return {
...
export default async function handler(req, res) {
  ...
  const payload = { prompt: body.prompt || '', videoMeta: body.videoMeta || {}, assets: body.assets || [], skill: body.skill || 'real-estate-pro', businessRules: body.businessRules || {} };
